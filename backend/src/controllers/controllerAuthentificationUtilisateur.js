...
const afficherToutUtilisateur = async (req,res)=>{
    try {
        ...
        return res.status(200).json({status:"valide",message:"Tous les utilisateurs sont affichés !",donnee:{...utilisateur,motdepasse:undefined}})
    } catch (error) {
        console.log(error)
        return res.status(500).json({message:"Erreur serveur",erreur:error.message})
    }
...
const majUtilisateurAdministrateur= async (req,res)=>{
    ...
    try {
        ...
        const donnees = {nom,prenom,email,motdepasse,contact,niveau,service}
        ...
        if(!utilisateur){
            return res.status(400).json({message:"Les données n'ont pas été mise à jour !"})
        }
        return res.status(200).json({status:"valide",message:"Les informations ont été mise à jour",donnee:{...utilisateur._doc,motdepasse:undefined}})
    } catch (error) {
        return res.status(500).json({message:"Erreur serveur !",erreur:error.message})
    }
}

const supprimerUtlisateur =async(req,res)=>{
    const {id} = req.body
    if(!id){
    ...
    try {
        ...
        return res.status(200).json({status:"valide",message:"L'utlisateur a bien été supprimé  !"})
    } catch (error) {
        return res.status(500).json({message:"Erreur serveur",erreur:error.message})
    }
}
